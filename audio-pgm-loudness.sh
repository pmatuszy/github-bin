...
# v. 20260801.145558 - YouTube PERFECT prompt defaults to no [y/N/q]
...
(video is ignored for speed). Each file is classified by peak level (max_volume):
  ...
  NO_PROCESS (mean below --mean-skip-db, default -60 dB)
                       Very low average level — not normalized unless you
                       answer Yes per file, or pass --force (with -y on CLI).
...
NO_PROCESS files are excluded unless --force is set.
...
GoPro MP4: gpmd/MET/SOS copied by stream index with -copy_unknown; tmcd is written in
pass1 (-write_tmcd) then copied at stream #2; MET follows at #3. GoPro files use a
two-pass remux so loudnorm does not drop the gpmd telemetry stream.
...
prompt_youtube_include_perfect_print_cli() {
  ...
  loudness_read_yn_key 'Include PERFECT-level files in YouTube normalize? [y/N/q]: ' N
  case "${REPLY^^}" in
    Q) loudness_quit_now ;;
...
loudness_print_run_summary_once() {
  ...
  local count_perfect=0 count_normal=0 count_too_quiet=0 count_no_process=0 count_no_audio=0 count_error=0 i
  ...
  if (( LOUDNESS_SCAN_RAN )); then
    ...
    if (( ${#ROW_STATUS[@]} > 0 )); then
      ...
      (( count_no_process > 0 )) && scan_line+=", ${count_no_process} no process"
...
# Peak class from max_volume; NO_PROCESS when mean_volume is below --mean-skip-db.
classify_loudness_status() {
  local max_db="$1" mean_db="$2" peak
  if loudness_mean_volume_below_skip_threshold "$mean_db"; then
    printf '%s' 'NO_PROCESS'
    return 0
  fi
  peak="$(classify_max_volume "$max_db")"
  printf '%s' "$peak"
}
...
loudness_skip_normalize_for_low_mean() {
  ...
  [[ "$status" == NO_PROCESS ]] && return 0
  loudness_mean_volume_below_skip_threshold "$mean_db"
}

loudness_normalize_prompt_default_no() {
  local status="$1"
  [[ "$status" == PERFECT || "$status" == NO_PROCESS ]]
}

loudness_count_queue_no_process_files() {
  local i n=0
  for i in "${!NORMALIZE_STATUS[@]}"; do
    [[ "${NORMALIZE_STATUS[$i]}" == NO_PROCESS ]] && (( ++n ))
  done
  printf '%s' "$n"
}
...
# Stream table for normalize (index|codec_type|codec_name|codec_tag|handler); set by normalize_load_stream_table.
...
normalize_stream_probe_tag() {
  local file="$1" idx="$2" tag="$3"
  local -a fp_args=(-v error)
...
# SMPTE timecode for GoPro pass1 -write_tmcd (video tag, else tmcd stream tag).
normalize_probe_file_timecode() {
  local file="$1" line idx ctype cname ctag handler tc

  tc="$(normalize_stream_probe_tag "$file" 0 timecode)"
  ...
  for line in "${NORMALIZE_STREAM_TABLE[@]}"; do
    ...
    tc="$(normalize_stream_probe_tag "$file" "$idx" timecode)"
    [[ -n "$tc" ]] && { echo "$tc"; return 0; }
  done
  return 1
}

normalize_file_stream_count() {
  local file="$1"
  local -a fp_args=(-v error)

  loudness_ffprobe_input_opts_for_file "$file" fp_args
  fp_args+=(-show_entries format=nb_streams -of default=nw=1:nk=1 -- "$file")
  ffprobe "${fp_args[@]}" 2>/dev/null || echo 0
}
...
normalize_load_stream_table() {
  ...
  while IFS= read -r idx; do
    ...
    handler="$(normalize_stream_handler_name "$file" "$idx")"
    NORMALIZE_STREAM_TABLE+=("${idx}|${ctype}|${cname}|${ctag}|${handler}")
...
normalize_file_needs_gopro_two_pass() {
  local src="$1" dest="$2"
  local mp4_family=0

  normalize_output_is_mp4_family "$dest" && mp4_family=1
  if ! (( mp4_family )); then
    normalize_output_is_mp4_family "$src" && mp4_family=1
  fi
  (( mp4_family )) || return 1
...
normalize_print_mux_note() {
  if (( NORMALIZE_GOPRO_TWO_PASS )); then
    echo '    Note: GoPro MP4 two-pass (loudnorm v/a+tmcd, then remux MET from source)'
...
# Pass 2: v/a/tmcd from pass1 input 1 (stream 2 = write_tmcd), copyable data from source input 0.
# Source tmcd is not stream-copied (codec none / tag issues); pass1 already muxed tmcd at #2.
normalize_build_gopro_two_pass_remux_args() {
  local -n out_map=$1
  local -n out_tag=$2
  local pass1_tmcd="${3:-0}"
...
normalize_file_inplace_gopro_two_pass() {
  ...
  tmp_norm="$(mktemp "${TMPDIR:-/tmp}/loudnorm.XXXXXX.mp4")"
  ref="$(mktemp)"
  stderr_log1="$(mktemp)"
  stderr_log2="$(mktemp)"
  touch -r "$src" "$ref"
  LOUDNESS_TMP_FILE="$tmp"
  NORMALIZE_GOPRO_TWO_PASS=1
  NORMALIZE_GOPRO_MUX=1

  mapfile -t encoder_args < <(normalize_audio_encoder_args "$dest")
  normalize_print_mux_note

  local timecode pass1_tmcd=0
  timecode="$(normalize_probe_file_timecode "$src" 2>/dev/null || true)"

  # Pass 1: loudnorm video/audio; mux SMPTE tmcd at stream #2 (source tmcd cannot be stream-copied).
  local -a pass1_extra=(-write_tmcd 1 -map_metadata 0 -movflags use_metadata_tags)
  [[ -n "$timecode" ]] && pass1_extra+=(-timecode "$timecode")
  ...
  if (( ffmpeg_rc != 0 )) || [[ ! -s "$tmp_norm" ]]; then
    print_ffmpeg_error_tail "$stderr_log1"
    rm -f -- "$tmp" "$tmp_norm" "$ref" "$stderr_log1" "$stderr_log2"
    LOUDNESS_TMP_FILE=""
    return 1
  fi
  rm -f -- "$stderr_log1"

  if [[ "$(normalize_file_stream_count "$tmp_norm")" -ge 3 ]]; then
    pass1_tmcd=1
  fi

  normalize_build_gopro_two_pass_remux_args map_args tag_args "$pass1_tmcd"
  ffmpeg_rc=0
  ...
  restore_file_timestamps_from_ref "$dest" "$ref"
  rm -f -- "$ref"
  return 0
}
...
normalize_build_ffmpeg_stream_map_args() {
  ...
  if ! (( mp4_family )); then
    normalize_output_is_mp4_family "$src_file" && mp4_family=1
  fi
...
normalize_file_inplace() {
  ...
  if normalize_file_needs_gopro_two_pass "$src" "$dest"; then
  ...
  # Map each stream; -filter:a loudnorm on every audio track; -c copy keeps video,
  # subtitles, attachments, gpmd, etc.; -c:a overrides audio to re-encode after filter.
  ffmpeg -hide_banner -nostats -y -i "$src" \
    "${map_args[@]}" \
    "${tag_args[@]}" \
    "${movflags_args[@]}" \
...
print_loudness_class_legend() {
  ...
  if [[ -n "$LOUDNESS_MEAN_SKIP_DB" ]]; then
    printf '  %-*s  %-*s  — %s\n' "$label_w" 'NO_PROCESS' "$range_w" "mean < ${LOUDNESS_MEAN_SKIP_DB} dB" 'do not normalize (--force)'
  fi
}
...
init_report_column_widths() {
  ...
  REPORT_STATUS_W=$(_table_disp_col_width "$REPORT_STATUS_W" 'NO_PROCESS')
...
print_scan_summary() {
  ...
  echo
  printf 'Summary: %d perfect, %d normal, %d too quiet, %d no process, %d no audio, %d error(s)\n' \
    "$count_perfect" "$count_normal" "$count_too_quiet" "$count_no_process" "$count_no_audio" "$count_error"

  if (( count_no_process > 0 )); then
    echo "NOTE: NO_PROCESS files have mean volume below ${LOUDNESS_MEAN_SKIP_DB} dB and are not normalization candidates (use --force to include)."
  fi
...
scan_one_media_file() {
  ...
  status="$(classify_loudness_status "$max_db" "$mean_db")"
...
prompt_youtube_include_perfect() {
  ...
  loudness_read_yn_key 'Include PERFECT files in YouTube normalize? [y/N/q]: ' N
  case "${REPLY^^}" in
    Q) loudness_quit_now ;;
    N) LOUDNESS_INCLUDE_PERFECT=0 ;;
    *)
...
normalize_skip_file_prompt() {
  local file="$1" status="${2:-}" mean_db="${3:-}"
  if (( AUTO_YES )) && ! loudness_skip_normalize_for_low_mean "$status" "$mean_db"; then
    return 0
  fi
...
loudness_read_normalize_batch_choice() {
  ...
  if ! loudness_normalize_prompt_default_no "$status"; then
    default='Y'
  fi
  max_disp="$(format_db_display_value "$max_db")"
  mean_disp="$(format_db_display_value "$mean_db")"

  if [[ "$status" == NO_PROCESS ]]; then
    echo "  NO_PROCESS (mean ${mean_disp} below ${LOUDNESS_MEAN_SKIP_DB} dB) — default [N] (CLI: --force with -y to include)."
  fi
...
normalize_run_batch_prompt_loop() {
  ...
  local file max_db mean_db status i j rc decision n_no_process=0
  ...
  n_no_process="$(loudness_count_queue_no_process_files)"
  if (( n_no_process > 0 )) && [[ -n "$LOUDNESS_MEAN_SKIP_DB" ]]; then
    if (( cli_only )); then
      :
    elif loudness_wants_per_file_prompts; then
      echo "NOTE: ${n_no_process} NO_PROCESS file(s) — per-file default [N] (use --force with -y to normalize all)."
    elif (( ! LOUDNESS_FORCE )); then
      echo "NOTE: ${n_no_process} NO_PROCESS file(s) will be skipped (use --force to include)."
    fi
  fi
  ...
  while (( idx < total )); do
    ...
    while (( idx < total && batch_count < batch_size_now )); do
      ...
      if [[ "$accept_all_remaining" == yes ]] || normalize_skip_file_prompt "$file" "$status" "$mean_db"; then
        batch_selected+=( 'yes' )
        (( ++batch_yes ))
        batch_indices+=( "$idx" )
        if (( cli_only )); then
...
normalize_candidate_files() {
  ...
  echo "GoPro MP4: two-pass remux (pass1 write_tmcd at #2, pass2 copies MET/gpmd from source)."
  if (( LOUDNESS_SAVE_ORIGINAL )); then
  ...
  if ! loudness_wants_per_file_prompts; then
    ...
    for i in "${!NORMALIZE_FILES[@]}"; do
      ...
      if ! normalize_skip_file_prompt "$file" "$status" "$mean_db"; then
        if [[ "$status" == NO_PROCESS ]]; then
          echo "    SKIP: NO_PROCESS (mean below ${LOUDNESS_MEAN_SKIP_DB} dB; use --force): ${file}"
        fi
        (( ++norm_skip ))
    ...
    echo '  Default: [Y] for NORMAL/TOO QUIET, [N] for PERFECT or NO_PROCESS.'
