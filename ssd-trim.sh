#!/bin/bash
# v. 20260922.203118 - fix HEALTHCHECK_URL unbound under set -u; only curl when URL is set
# v. 20260811.095711 - add --history (paged changelog via _script_header.sh print_script_history)
# v. 20260716.164840 - add -h/--help, -v/--version, --no_startup_delay

# 2026.09.22 - v. 0.7.203118 - Initialize HEALTHCHECK_URL; ping healthchecks only when a URL is configured (avoids unbound variable when healthchecks-ids.txt is missing or has no entry)
...
# ssd-trim.sh
#
# Run fstrim --all and optionally report status to healthchecks.io.
#
