...
export URL="https://gopro.com/en/us/update/hero10-black"
timeout=300
kill_after=310
rozmiar_x_ekran=900
rozmiar_y_ekran=700
...
convert "${plik_bez_cropa}" -crop 800x800+200+50 "${plik_po_cropie}"

echo "${URL}" > "${zawartosc_maila}"

mpack -s "(`date '+%Y.%m.%d %H:%M'`) Gopro10 firmware update" -c image/jpeg "${plik_po_cropie}" -d "${zawartosc_maila}" matuszyk@matuszyk.com
