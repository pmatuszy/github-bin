...
convert "${plik_bez_cropa}" -crop 800x800+50+50 "${plik_po_cropie}"

echo "${URL}" > "${zawartosc_maila}"

mpack -s "(`date '+%Y.%m.%d %H:%M'`) nuci7 firmware update" -c image/jpeg "${plik_po_cropie}" -d "${zawartosc_maila}" matuszyk@matuszyk.com
